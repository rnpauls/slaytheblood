...
func take_damage(damage: int, which_modifier: Modifier.Type, damage_kind: Card.DamageKind = Card.DamageKind.PHYSICAL, prevention: int = -1) -> int:
	if stats.health <= 0:
		return 0

	sprite_2d.material = WHITE_SPRITE_MATERIAL
	var modified_damage := modifier_handler.get_modified_value(damage, which_modifier)
	var damage_taken := stats.take_damage(modified_damage, damage_kind, prevention)
