...
func execute(targets: Array[Node]) -> bool:
	...
	for target in targets:
		...
		if target is Enemy:
			...
			hand_only_sixes = original_hand.filter(func(card: Card): return card.attack > 5)
			hand_non_sixes = original_hand.filter(func(card: Card): return card.attack < 6)
		
		var num_non_sixes:= amount - hand_only_sixes.size()
		if num_non_sixes > 0:
			all_discards_are_six = false
			for card in hand_only_sixes:
				discard_card_from_target(target, card)
...
func discard_card_from_target(target: Enemy, card) -> void:
	if target:
		card.discard_card()
